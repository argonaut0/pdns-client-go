/*
PowerDNS Authoritative HTTP API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.0.15
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the SimpleStatisticItem type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SimpleStatisticItem{}

// SimpleStatisticItem struct for SimpleStatisticItem
type SimpleStatisticItem struct {
	// Item name
	Name *string `json:"name,omitempty"`
	// Item value
	Value *string `json:"value,omitempty"`
}

// NewSimpleStatisticItem instantiates a new SimpleStatisticItem object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSimpleStatisticItem() *SimpleStatisticItem {
	this := SimpleStatisticItem{}
	return &this
}

// NewSimpleStatisticItemWithDefaults instantiates a new SimpleStatisticItem object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSimpleStatisticItemWithDefaults() *SimpleStatisticItem {
	this := SimpleStatisticItem{}
	return &this
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *SimpleStatisticItem) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SimpleStatisticItem) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *SimpleStatisticItem) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *SimpleStatisticItem) SetName(v string) {
	o.Name = &v
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *SimpleStatisticItem) GetValue() string {
	if o == nil || IsNil(o.Value) {
		var ret string
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SimpleStatisticItem) GetValueOk() (*string, bool) {
	if o == nil || IsNil(o.Value) {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *SimpleStatisticItem) HasValue() bool {
	if o != nil && !IsNil(o.Value) {
		return true
	}

	return false
}

// SetValue gets a reference to the given string and assigns it to the Value field.
func (o *SimpleStatisticItem) SetValue(v string) {
	o.Value = &v
}

func (o SimpleStatisticItem) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SimpleStatisticItem) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Value) {
		toSerialize["value"] = o.Value
	}
	return toSerialize, nil
}

type NullableSimpleStatisticItem struct {
	value *SimpleStatisticItem
	isSet bool
}

func (v NullableSimpleStatisticItem) Get() *SimpleStatisticItem {
	return v.value
}

func (v *NullableSimpleStatisticItem) Set(val *SimpleStatisticItem) {
	v.value = val
	v.isSet = true
}

func (v NullableSimpleStatisticItem) IsSet() bool {
	return v.isSet
}

func (v *NullableSimpleStatisticItem) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSimpleStatisticItem(val *SimpleStatisticItem) *NullableSimpleStatisticItem {
	return &NullableSimpleStatisticItem{value: val, isSet: true}
}

func (v NullableSimpleStatisticItem) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSimpleStatisticItem) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


